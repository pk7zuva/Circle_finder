...
awk '$1>2 {print $2}' $6-$7\.split.id-freq.txt > $6-$7\.split.id-freqGr2.txt
...
awk '{print $4}' $6-$7\.disc.txt | sort | uniq -c > $6-$7\.disc.id-freq.txt
awk '$1=="2" {print $2}' $6-$7\.disc.id-freq.txt > $6-$7\.disc.id-freq2.txt
awk '$1>2 {print $2}' $6-$7\.disc.id-freq.txt > $6-$7\.disc.id-freqGr2.txt
...
grep -w -Ff $6-$7\.split.id-freqGr2.txt $6-$7\.split.txt > $6-$7\.split_freqGr2.txt
...
#Selecting dis-cordant pairs that were 1) mapped uniquely and 2) mapped on more than one loci. The file "$6-$7\.disc_freq2.txt" may be used together with file "$6-$7\_split_freqGr2.txt" to identify additional microDNA
grep -w -Ff $6-$7\.disc.id-freq2.txt $6-$7\.disc.txt > $6-$7\.disc_freq2.txt
grep -w -Ff $6-$7\.disc.id-freqGr2.txt $6-$7\.disc.txt > $6-$7\.disc_freqGr2.txt
...
awk '$1==$9 && $7==$15 && $6>0 && $14>0 {print $4} ' $6-$7\.split_freq2.oneline.txt > $6-$7\.split_freq2.oneline.S-R-S-CHR-S-ST.ID.txt
...
awk '{printf ("%s\t%d\n",$0,($3-$2)+1)}' $6-$7\.concordant_freq3.2SPLIT-1M.txt | sort -k4,4 -k9,9n | sed 'N;N;s/\n/\t/g' > $6-$7\.concordant_freq3.2SPLIT-1M.inoneline.txt
